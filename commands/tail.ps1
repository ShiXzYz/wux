function tail {
    ...
    process {
        ...
        function Get-Tail {
            ...
            if ($fromStart) { return $content | Select-Object -Skip ($lineCount - 1) }
            else             { return $content | Select-Object -Last  $lineCount }
        }
        ...
        if ($Files.Count -eq 0) {
            $all    = $pipeLines.ToArray()
            $sliced = if ($fromStart) { $all | Select-Object -Skip ($lineCount - 1) }
                      else             { $all | Select-Object -Last  $lineCount }
